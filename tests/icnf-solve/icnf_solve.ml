...
module Parse : sig
  ...
  val next : 'a t -> 'a event
  (** @raise End_of_file when done *)
end = struct
  module L = Lexer
...
let solve_with_solver ~check ~debug file : unit =
  ...
  let rec process_problem () =
    ...
    | Parse.Solve assumptions ->
      if debug then Printf.printf "c solve %a\n%!" pp_arr assumptions;
      ( match Solver.solve s assumptions with
      ...
      | Error (_ :: _ as core) when check ->
        ...
        (* check unsat core *)
        ( match Solver.solve s core with
        | Ok () ->
          Printf.printf "error: unsat core %a is SAT\n%!" pp_arr core;
          exit 1
        | Error _ -> () )
      | Error _ -> Printf.printf "UNSAT\n%!" );
