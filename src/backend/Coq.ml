...
struct
  ...
  let elim_duplicate fmt goal hyp _ =
    ...
    Format.fprintf fmt "pose proof @[<hov>(fun %a=>@ %s%a) as %s@].@\n"
      (clause_iter m "%s@ ") goal (name hyp) (clause_iter m "@ %s") hyp
      (name goal)
  ...
  let resolution_aux m a h1 h2 fmt () =
    Format.fprintf fmt "%s%a" (name h1)
      (fun fmt ->
        Array.iter (fun b ->
            if b == a then
              Format.fprintf fmt "@ (fun p =>@ %s%a)" (name h2)
                (fun fmt ->
                  Array.iter (fun c ->
  ...
  let prove_node t fmt node =
    ...
    | P.Hypothesis _ -> A.prove_hyp fmt (name clause) clause
    | P.Assumption -> A.prove_assumption fmt (name clause) clause
    | P.Lemma _ -> A.prove_lemma fmt (name clause) clause
    | P.Duplicate (p, l) ->
  ...
  let count_uses p =
    ...
    let aux () node =
      List.iter
        (fun p' -> incr_use h P.(conclusion p'))
        (P.parents node.P.step)
    in
    let () = P.fold aux () p in
    h
  ...
  let pp fmt p =
    ...
    let aux () node = Format.fprintf fmt "%a" (prove_node h) node in
...
module Simple
  ...
  Make
    ...
    (struct
      ...
      let prove_lemma fmt name c = A.prove_lemma fmt name (get_lemma c)
