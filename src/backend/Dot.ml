...
struct
  ...
  let print_edges fmt n =
    ...
    | P.Hyper_res { P.hr_init; hr_steps = (_, p0) :: _ as l } ->
      ...
      List.iter (fun (_, p2) -> print_edge fmt (res_np_id n p2) (proof_id p2)) l
    | _ -> ()
  ...
  let table fmt (c, rule, color, l) =
    ...
    match l with
    | [] ->
      Format.fprintf fmt "<TR><TD BGCOLOR=\"%s\" colspan=\"2\">%s</TD></TR>"
        color rule
    | f :: r ->
      Format.fprintf fmt
        "<TR><TD BGCOLOR=\"%s\" rowspan=\"%d\">%s</TD><TD>%a</TD></TR>" color
        (List.length l) rule f ();
...
module Simple
  ...
  Make
    ...
    (struct
      ...
      (* Actual functions *)
      let print_atom fmt a = A.print_atom fmt (lit a)
      let hyp_info c = A.hyp_info (List.map lit (S.Clause.atoms_l c))
      let lemma_info c = A.lemma_info (get_lemma c)
      let assumption_info c = A.assumption_info (lit (get_assumption c))
    end)
