...
module type PLUGIN = sig
  ...
  val has_theory : bool
  (** Is this a CDCL(T) plugin or mcsat plugin? i.e does it have theories *)
...
module Make (Plugin : PLUGIN) = struct
  ...
  module Lit = struct
    ...
    let debug_assign fmt v =
      ...
      | Some t -> Format.fprintf fmt "@[<hov>@@%d->@ %a@]" v.l_level Value.pp t

    let pp out v = Term.pp out v.term

    let debug out v =
      Format.fprintf out "%d[%a][lit:@[<hov>%a@]]" (v.lid + 1) debug_assign v
        Term.pp v.term
  end
  ...
  module Var = struct
    ...
    let[@inline] set_default_pol_true v =
      v.v_fields <- v.v_fields lor default_pol_true

    let[@inline] set_default_pol_false v =
      v.v_fields <- v.v_fields land lnot default_pol_true

    let[@inline] default_pol v = v.v_fields land default_pol_true <> 0

    let make ?(default_pol = true) (st : st) (t : formula) :
        var * Solver_intf.negated =
    ...
    let[@inline] seen_both v =
      seen_pos land v.v_fields <> 0 && seen_neg land v.v_fields <> 0
  end
  ...
  module Atom = struct
    ...
    let[@inline] seen a =
      if sign a then
        seen_pos land a.var.v_fields <> 0
      else
        seen_neg land a.var.v_fields <> 0
    ...
    let debug_reason fmt = function
      | n, _ when n < 0 -> Format.fprintf fmt "%%"
      | n, None -> Format.fprintf fmt "%d" n
      | n, Some Decision -> Format.fprintf fmt "@@%d" n
      | n, Some (Bcp c) -> Format.fprintf fmt "->%d/%s" n (name_of_clause c)
      | n, Some (Bcp_lazy _) -> Format.fprintf fmt "->%d/<lazy>" n
      | n, Some Semantic -> Format.fprintf fmt "::%d" n

    let pp_level fmt a = debug_reason fmt (a.var.v_level, a.var.reason)
    ...
    let debug out a =
      Format.fprintf out "%s%d[%a][atom:@[<hov>%a@]]" (pp_sign a)
        (a.var.vid + 1) debug_value a Formula.pp a.lit

    let debug_a out vec =
      Array.iter (fun a -> Format.fprintf out "%a@ " debug a) vec

    let debug_l out l = List.iter (fun a -> Format.fprintf out "%a@ " debug a) l
  ...
  module Elt = struct
    ...
    let[@inline] set_level e lvl =
      match e with
      | E_lit l -> l.l_level <- lvl
      | E_var v -> v.v_level <- lvl

    let[@inline] set_idx e i =
      match e with
      | E_lit l -> l.l_idx <- i
      | E_var v -> v.v_idx <- i

    let[@inline] set_weight e w =
      match e with
      | E_lit l -> l.l_weight <- w
      | E_var v -> v.v_weight <- w
  end
  ...
  module Clause = struct
    ...
    let make_a =
      ...
      fun ~flags atoms premise ->
        let cid = !n in
        incr n;
        { cid; atoms; flags; activity = 0.; cpremise = premise }
    ...
    let[@inline] make_removable_a l premise =
      make_a ~flags:flag_removable l premise
    ...
    let[@inline] set_visited c b =
      if b then
        c.flags <- c.flags lor flag_visited
      else
        c.flags <- c.flags land lnot flag_visited

    let[@inline] attached c = c.flags land flag_attached <> 0

    let[@inline] set_attached c b =
      if b then
        c.flags <- c.flags lor flag_attached
      else
        c.flags <- c.flags land lnot flag_attached

    let[@inline] removable c = c.flags land flag_removable <> 0

    let[@inline] set_removable c b =
      if b then
        c.flags <- c.flags lor flag_removable
      else
        c.flags <- c.flags land lnot flag_removable
  ...
  module Proof = struct
    ...
    let error_res_f msg =
      Format.kasprintf (fun s -> raise (Resolution_error s)) msg
    ...
    let rec set_atom_proof a =
      let aux acc b =
        if Atom.equal a.neg b then
          acc
        else
          set_atom_proof b :: acc
      in
      assert (a.var.v_level >= 0);
      match a.var.reason with
      | Some (Bcp c | Bcp_lazy (lazy c)) ->
        Log.debugf 5 (fun k ->
            k "(@[proof.analyze.clause@ :atom %a@ :c %a@])" Atom.debug a
              Clause.debug c);
        if Array.length c.atoms = 1 then (
          Log.debugf 5 (fun k ->
              k "(@[proof.analyze.old-reason@ %a@])" Atom.debug a);
          c
        ) else (
          ...
          Log.debugf 5 (fun k ->
              k "(@[proof.analyze.new-reason@ :atom %a@ :c %a@])" Atom.debug a
                Clause.debug c');
          c'
        )
      | _ -> error_res_f "cannot prove atom %a" Atom.debug a
    ...
    let prove_unsat conflict =
      if Array.length conflict.atoms = 0 then
        ...
        Log.debugf 1 (fun k ->
            k "(@[sat.prove-unsat@ :from %a@])" Clause.debug conflict);
        let l =
          Array.fold_left
            (fun acc a -> set_atom_proof a :: acc)
            [] conflict.atoms
        in
    ...
    let find_pivots (init : clause) (l : clause list) : _ * (atom * t) list =
      Log.debugf 15 (fun k ->
          k "(@[proof.find-pivots@ :init %a@ :l %a@])" Clause.debug init
    ...
    let expand conclusion =
      Log.debugf 5 (fun k ->
          k "(@[sat.proof.expand@ @[%a@]@])" Clause.debug conclusion);
      match conclusion.cpremise with
      | Lemma l -> { conclusion; step = Lemma l }
      | Local -> { conclusion; step = Assumption }
      | Hyp l -> { conclusion; step = Hypothesis l }
      | History [] ->
    ...
    let is_leaf = function
      | Hypothesis _ | Assumption | Lemma _ -> true
      | Duplicate _ | Hyper_res _ -> false
  ...
  module H = Heap.Make [@specialise] (struct
    ...
    let[@inline] cmp i j =
      Elt.weight j < Elt.weight i (* comparison by weight *)
  ...
  type t = {
    ...
    elt_levels: int Vec.t; (* decision levels in [trail]  *)
    mutable assumptions: atom Vec.t; (* current assumptions *)
    mutable th_head: int;
  ...
  let create ?on_conflict ?on_decision ?on_new_atom ?(store_proof = true)
      ?(size = `Big) (th : theory) : t =
  ...
  let mk_atom ?default_pol st (f : formula) : atom =
    ...
    if Plugin.mcsat then mk_atom_mcsat_ st res;
    res
  ...
  *)
  ...
  and insert_var_order st (v : var) : unit = insert_elt_order st (E_var v)
  ...
  let var_bump_activity_aux st v =
    ...
    if H.in_heap elt then H.decrease st.order elt
    ...
    let rec partition_aux trues unassigned falses history i =
      if i >= Array.length atoms then
        ...
        if a.is_true then (
          ...
          if l = 0 then
            raise
              Trivial (* A var true at level 0 gives a trivially true clause *)
          else
  ...
  *)
  ...
  let report_unsat st (us : unsat_cause) : _ =
    Log.debugf info (fun k ->
        k "(@[sat.unsat-conflict@ %a@])" pp_unsat_cause us);
    let us =
      match us with
      | US_false c ->
        let c =
          if st.store_proof then
            Proof.prove_unsat c
          else
            c
        in
  ...
  let enqueue_bool st a ~level:lvl reason : unit =
    if a.neg.is_true then (
      Log.debugf error (fun k ->
          k "(@[sat.error.trying to enqueue a false literal %a@])" Atom.debug a);
      assert false
    );
    assert (
      (not a.is_true) && a.var.v_level < 0 && a.var.reason = None && lvl >= 0);
    let reason =
      if lvl > 0 then
        reason
      else
    ...
    Log.debugf debug (fun k ->
        k "(@[sat.enqueue[%d]@ %a@])" (Vec.size st.trail) Atom.debug a);
    ()
  ...
  let enqueue_assign st (l : lit) (value : value) lvl =
    ...
    | Some _ ->
      Log.debugf error (fun k ->
          k "(@[sat.error: Trying to assign an already assigned literal:@ %a@])"
            Lit.debug l);
      assert false
    | None ->
      assert (l.l_level < 0);
      ...
      Log.debugf debug (fun k ->
          k "(@[sat.enqueue-semantic[%d]@ %a@])" (Vec.size st.trail) Lit.debug l);
      ()
  ...
  let th_eval st a : bool option =
    if a.is_true || a.neg.is_true then
      None
    else (
      match Plugin.eval st.th a.lit with
      | Solver_intf.Unknown -> None
      | Solver_intf.Valued (b, l) ->
        if l = [] then
          invalid_argf "semantic propagation at level 0 currently forbidden: %a"
            Atom.pp a;
        let atom =
          if b then
            a
  ...
  let analyze st c_clause : conflict_res =
    ...
    Vec.clear to_unmark;
    let conflict_level =
      if Plugin.mcsat || Plugin.has_theory then
    ...
    while !cond do
      ...
      | Some clause ->
        Log.debugf debug (fun k ->
            k "(@[sat.analyze-conflict.resolve@ %a@])" Clause.debug clause);
        if Clause.removable clause then clause_bump_activity st clause;
        history := clause :: !history;
        (* visit the current predecessors *)
        for j = 0 to Array.length clause.atoms - 1 do
          let q = clause.atoms.(j) in
          assert (q.is_true || (q.neg.is_true && q.var.v_level >= 0));
          (* unsure? *)
      ...
      while
        ...
        Log.debugf debug (fun k ->
            k "(@[sat.analyze-conflict.at-trail-elt@ %a@])" Trail_elt.debug a);
        match a with
        | Atom q -> (not (Var.marked q.var)) || q.var.v_level < conflict_level
        | Lit _ -> true
      do
        decr tr_ind
      done;
  ...
  *)
  let add_boolean_conflict st (confl : clause) : unit =
    Log.debugf info (fun k ->
        k "(@[sat.add-bool-conflict@ %a@])" Clause.debug confl);
    st.next_decisions <- [];
    assert (decision_level st >= 0);
    if
      decision_level st = 0
      || Array.for_all (fun a -> a.var.v_level <= 0) confl.atoms
    then
      (* Top-level conflict *)
      report_unsat st (US_false confl);
  ...
  let add_clause_ st (init : clause) : unit =
    Log.debugf debug (fun k ->
        k "(@[sat.add-clause@ @[<hov>%a@]@])" Clause.debug init);
    ...
    try
      ...
      Log.debugf info (fun k ->
          k "(@[sat.new-clause@ @[<hov>%a@]@])" Clause.debug clause);
      match atoms with
      ...
      | [ a ] ->
        ...
        (* atom is already true, nothing to do *)
          ...
          Log.debugf debug (fun k ->
              k "(@[sat.add-clause.unit-clause@ :propagating %a@])" Atom.debug a);
    ...
    with Trivial ->
      Log.debugf info (fun k ->
          k "(@[sat.add-clause@ :ignore-trivial @[%a@]@])" Clause.debug init)
  ...
  *)
  ...
  let slice_get st i =
    match Vec.get st.trail i with
    | Atom a -> Solver_intf.Lit a.lit
    | Lit { term; assigned = Some v; _ } -> Solver_intf.Assign (term, v)
    | Lit _ -> assert false

  let acts_add_clause st ?(keep = false) (l : formula list) (lemma : lemma) :
      unit =
  ...
  let acts_add_decision_lit (st : t) (f : formula) (sign : bool) : unit =
    ...
    if not (Atom.has_value a) then (
      Log.debugf 10 (fun k ->
          k "(@[sat.th.add-decision-lit@ %a@])" Atom.debug a);
  ...
  let acts_raise st (l : formula list) proof : 'a =
    ...
    Log.debugf 5 (fun k ->
        k "(@[@{<yellow>sat.th.raise-conflict@}@ %a@])" Clause.debug c);
  ...
  let check_consequence_lits_false_ l : unit =
    ...
    | a ->
      invalid_argf
        "slice.acts_propagate:@ Consequence should contain only true literals, \
         but %a isn't"
  ...
  let[@inline] current_slice st : _ Solver_intf.acts =
    {
      Solver_intf.acts_iter_assumptions = acts_iter st ~full:false st.th_head;
  ...
  let[@inline] full_slice st : _ Solver_intf.acts =
    {
      Solver_intf.acts_iter_assumptions = acts_iter st ~full:true st.th_head;
      acts_eval_lit = acts_eval_lit st;
      acts_mk_lit = acts_mk_lit st;
      acts_mk_term = acts_mk_term st;
      acts_add_clause = acts_add_clause st;
    ...
    if st.elt_head = Vec.size st.trail then
      ...
      match
        while st.elt_head < Vec.size st.trail do
          (match Vec.get st.trail st.elt_head with
          | Lit _ -> ()
  ...
  let analyze_final (self : t) (a : atom) : atom list =
    ...
    while !idx >= 0 do
      (match Vec.get self.trail !idx with
    ...
    Log.debugf 5 (fun k ->
        k "(@[sat.analyze-final.done@ :core %a@])"
          (Format.pp_print_list Atom.debug)
          !core);
    !core
  ...
  let reduce_db (st : t) (n_of_learnts : int) : unit =
    ...
    Log.debugf 3 (fun k ->
        k "(@[sat.gc.start :keep %d :out-of %d@])" n_of_learnts (Vec.size v));
  ...
  and pick_branch_lit st =
    ...
    | [] ->
      (match H.remove_min st.order with
      | E_lit l ->
  ...
  let search (st : t) n_of_conflicts n_of_learnts : unit =
    Log.debugf 3 (fun k ->
        k "(@[sat.search@ :n-conflicts %d@ :n-learnt %d@])" n_of_conflicts
          n_of_learnts);
  ...
  let solve_ (st : t) : unit =
    Log.debugf 5 (fun k ->
        k "(@[sat.solve :assms %d@])" (Vec.size st.assumptions));
    check_unsat_ st;
    try
      flush_clauses st;
      (* add initial clauses *)
      ...
      let n_of_learnts =
        ref (float_of_int (nb_clauses st) *. learntsize_factor)
      in
      while true do
        try
        ...
        | E_sat ->
          assert (
            ...
            && st.next_decisions = []);
          (match Plugin.final_check st.th (full_slice st) with
          | () ->
            if
              st.elt_head = Vec.size st.trail
              && Vec.is_empty st.clauses_to_add
          ...
          | exception Th_conflict c ->
            ...
            Array.iter (fun a -> insert_elt_order st (Elt.of_var a.var)) c.atoms;
            Log.debugf info (fun k ->
                k "(@[sat.theory-conflict-clause@ %a@])" Clause.debug c);
            (match st.on_conflict with
            | Some f -> f c.atoms
            | None -> ());
            Vec.push st.clauses_to_add c;
  ...
  let assume st cnf lemma =
    List.iter
      (fun l ->
        ...
        Log.debugf debug (fun k ->
            k "(@[sat.assume-clause@ @[<hov 2>%a@]@])" Clause.debug c);
        Vec.push st.clauses_to_add c)
      cnf
  ...
  let check_clause c =
    ...
    if not res then (
      Log.debugf debug (fun k ->
          k "(@[sat.check-clause@ :not-satisfied @[<hov>%a@]@])" Clause.debug c);
      false
    ) else
      true
    ...
    Log.debugf lvl (fun k ->
        k
          "(@[<v>sat.full-state :res %s - Full summary:@,\
           ...
           @]@."
          status
          (Vec.pp ~sep:"" Trail_elt.debug)
          (trail st)
          (Vec.pp ~sep:"" Clause.debug)
          (hyps st)
          (Vec.pp ~sep:"" Clause.debug)
          (history st))
  ...
  let mk_sat (st : t) : (Term.t, Formula.t, _) Solver_intf.sat_state =
    ...
    { Solver_intf.eval; eval_level; iter_trail; model = (fun () -> model st) }
        ...
        let c =
          lazy
            (let core = List.rev core in
             ...
             in
             let other_lits =
               List.filter (fun a -> not (Atom.equal a first)) core
             in
             let hist =
               Clause.make_permanent [ first ] Local
               :: proof_of first
               :: List.map proof_of other_lits
             in
  ...
  let add_clause_a st c lemma : unit =
    try
      ...
      add_clause_ st c
    with E_unsat (US_false c) -> st.unsat_at_0 <- Some c
  ...
  let solve ?(assumptions = []) (st : t) : res =
    ...
    try
      solve_ st;
      Sat (mk_sat st)
    with E_unsat us -> Unsat (mk_unsat st us)
...
module Make_cdcl_t (Plugin : Solver_intf.PLUGIN_CDCL_T) = Make (struct
  include Plugin
  module Term = Void_
  module Value = Void_
...
module Make_mcsat (Plugin : Solver_intf.PLUGIN_MCSAT) = Make (struct
  include Plugin
