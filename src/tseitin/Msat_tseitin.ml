...
module Make (F : Tseitin_intf.Arg) = struct
  ...
  let rec flatten comb acc = function
    | [] -> acc
    | Comb (c, l) :: r when c = comb -> flatten comb (List.rev_append l acc) r
    | a :: r -> flatten comb (a :: acc) r
  ...
  and sform_list acc l k =
    match l with
    | [] -> k acc
    | f :: tail -> sform f (fun f' -> sform_list (f' :: acc) tail k)

  and sform_list_not acc l k =
    match l with
    | [] -> k acc
    | f :: tail ->
