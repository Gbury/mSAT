...
(** Atoms are represented as integers. [-i] begin the negation of [i].
    Additionally, since we nee dot be able to create fresh atoms, we use even
    integers for user-created atoms, and odd integers for the fresh atoms. *)
...
(** Internal function for creating atoms. Updates the internal counters *)
let _make i =
